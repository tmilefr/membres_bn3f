...
{	
	...
	public function RenderFormElement(){
		...
		return '<input data-dst="input'.$clean_name.$this->parent_id.'" name="ta'.$this->name.'" id="ta'.$clean_name.'" data-source="'.base_url().$this->RenderTools->_get('_controller_name').'/JsonData/'.(($this->json_url) ? $this->json_url:$clean_name).'"  autocomplete="off" class="typeahead form-control" value="'.((isset($this->values[$this->value])) ? $this->values[$this->value] : '').'" placeholder="'.Lang('help_'.$this->name).'"><input type="hidden" id="input'.$clean_name.$this->parent_id.'" name="'.$this->name.'" value="'.$this->value.'">';
	}
